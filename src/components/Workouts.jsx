...
export default function Workouts() {
  ...
  const addDayToNewPlan = () => {
    setNewPlan((prev) => ({
      ...
      days: [
        ...prev.days,
        { name: `Day ${prev.days.length + 1}`, exercises: [] },
      ],
    }));
  };
  ...
  const handleDaySelect = (day) => {
    ...
    day.exercises.forEach((ex) => {
      initialLogs[ex.exercise_id] = Array.from(
        { length: ex.sets || 3 },
        () => ({
          weight: ex.weight || '',
          reps: ex.reps || '',
    ...
    <Box sx={{ flexGrow: 1 }}>
      ...
      {isCreatingPlan ? (
        <Paper sx={{ p: 3 }}>
          ...
          <Button variant="outlined" sx={{ mr: 2 }} onClick={addDayToNewPlan}>
            Add Another Day
          </Button>
                      ...
                      <TableBody>
                        ...
                        {Object.values(
                          session.logs.reduce((acc, log) => {
                            ...
                            acc[log.exercise_id].maxWeight = Math.max(
                              acc[log.exercise_id].maxWeight,
                              log.weight
                            );
                            return acc;
                          }, {})
